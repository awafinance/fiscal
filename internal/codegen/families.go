	...
	return runGeneratedPostprocess(verbose, generatedPostprocessSpec{
		...
		workarounds: []generatedWorkaround{
			...
			workaround("restore typed UF on CTe EPEC toma4 payloads", postprocess.IfPath(
				func(path string) bool {
					return strings.HasSuffix(path, string(filepath.Separator)+"evEPECCTe_v4.00.xsd.go")
				},
				postprocess.ReplaceFieldType(postprocess.AllFields(postprocess.FieldNamed("UF"), postprocess.FieldTypeEquals("string")), "*TUf"),
			)),
			workaround("fix typed root aliases from schema definitions", fixTypedRootAliasFromSchema),
