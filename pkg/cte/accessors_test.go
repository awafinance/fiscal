...
func TestDocumentGetEmitterDetailCTeOS(t *testing.T) {
	...
	detail := doc.GetEmitterDetail()
	require.NotNil(t, detail)
	require.Equal(t, "999999999999", detail.IE)
	require.Empty(t, detail.TradeName)
	require.Equal(t, "9999999999", detail.Phone)

	require.NotNil(t, detail.Address)
	require.Equal(t, "XXXXXXXXXXXXXXXX", detail.Address.Street)
	require.Equal(t, "720", detail.Address.Number)
	require.Equal(t, "XXXXXXXXXXX", detail.Address.Neighborhood)
	require.Equal(t, "3534401", detail.Address.CityCode)
	require.Equal(t, "SAO PAULO", detail.Address.CityName)
	require.Equal(t, "SP", detail.Address.State)
	require.Equal(t, "99999999", detail.Address.ZipCode)
}
